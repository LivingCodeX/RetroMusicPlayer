...
object AutoGeneratedPlaylistBitmap {
    ...
    fun getBitmap(context: Context, songPlaylist: List<Song>?): Bitmap? {
        ...
        return if (PreferenceUtil.isIgnoreMediaStoreArtwork) {
            if (songPlaylist.size == 1) return getBitmapWithSong(context, songPlaylist[0])
            val art: MutableList<Bitmap> = ArrayList()
            for (song in songPlaylist) {
                val bitmap = getBitmapWithSong(context, song)
    ...
    private fun getBitmapWithSong(context: Context, song: Song): Bitmap? {
        return try {
            Glide.with(context)
                .asBitmap()
                .load(AudioFileCover(song.data))
                .submit(200, 200)
                .get()
        } catch (e: Exception) {
            null
